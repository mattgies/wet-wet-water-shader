var basic_frag_shader = `
		...
		void main() {
			...
			float yDisplacement = (dispMapOffset1 + dispMapOffset2) / 10.0;
			...
			vec3 waterNormal = waterNormalInCamSpace;
			...
			float I = 0.3;
			vec3 v_vNorm = normalize(vec3(u_nMatrix * vec4(waterNormal, 1.0)));
			float maxDot = max(0.0, dot(vec3(0.0, 1.0, 0.0), camSpaceLightPos - v_vPos));
			float rSquared = length ( newt * newRefractRay.y ) * length ( newt * newRefractRay.y );
            
            gl_FragColor = 1.8 * texture2D(u_groundColorMap, v_vTexCoordsOriginal);
			gl_FragColor = gl_FragColor * vec4((I / rSquared * maxDot * Kd), 1.0); // multiplicative blending between base colors
			gl_FragColor = gl_FragColor + 0.5 * caustic; // additive blending between base colors + white highlights from caustics
		}
	`; 